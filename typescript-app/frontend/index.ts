...
document.addEventListener('DOMContentLoaded', function() {
    getChatListFromApi();
    let message_send_button = document.getElementById('message-send-button') as HTMLButtonElement;
    message_send_button.addEventListener("click", sendMessageToCustomer);
    let add_attachment_button = document.getElementById('attachment-send-button') as HTMLButtonElement;
    add_attachment_button.addEventListener('click', FileUpload);


    let message_input = document.getElementById("message-input") as HTMLTextAreaElement
...
function FileUpload() {
    let attachment_input = document.getElementById('file-input') as HTMLInputElement;
...
const acceptedPhotoExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff'];
...
function sendMessageToCustomer() {
    ...
    let message_text_input = document.getElementById('message-input') as HTMLInputElement;
    ...
    if (file_attachment_name) {
        ...
        } else if (acceptedAudioExtensions.some(ext => lowerCaseName.endsWith(ext))) {
            target_api_method = 'new-audio-message';
        } else if (acceptedVideoExtensions.some(ext => lowerCaseName.endsWith(ext))) {
...
function displayDialogList(chat_list: ChatListContainer) {
    ...
    if (contactList) {contactList.appendChild(ContactItem);}
    });
    ...
    contactListItems.forEach((li) => {
        ...
        li.addEventListener('click', (event) => {
            ...
            const chatSettingFullName = document.getElementById('chat-setting-full-name');

            let uniqueNumber = Number((event.currentTarget as HTMLElement).id.split('_')[1]);
            ...
            if (Number(active_chat) !== Number(uniqueNumber)) {
                ...
                let chatNameElement = (event.currentTarget as HTMLElement).querySelector('.chat-name');
                let chatName = chatNameElement ? chatNameElement.textContent : 'Default Name';


                let chatPhotoElement = (event.currentTarget as HTMLElement).querySelector('.chat-photo') as HTMLImageElement;
                let chatPhotoSrc = '/tg-bot/file?file_uuid=' + chatPhotoElement ? chatPhotoElement.src : "/user_empty_photo.png";

                if (chatSettingFullName) {chatSettingFullName.textContent = chatName;}
                if (chatPhotoElement) {chatPhotoElement.src = chatPhotoSrc;}

                var chatSettingTopPanel = document.getElementById(`top_chat_setting_panel`);
                ...
                var inputPanel = document.getElementById('input_chat_box') as HTMLDivElement;
