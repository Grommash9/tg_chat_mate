...
async def video_message_from_user(message: types.Message) -> None:
    attachment_file_id = await upload_file_to_db_using_file_id(message.video.file_id)
    message_document = db.message.new_message(message, unread=True, attachment=attachment_file_id)
    try:
...
async def video_note_message_from_user(message: types.Message) -> None:
    attachment_file_id = await upload_file_to_db_using_file_id(
...
async def voice_message_from_user(message: types.Message) -> None:
    attachment_file_id = await upload_file_to_db_using_file_id(message.voice.file_id)
    message_document = db.message.new_message(message, unread=True, attachment=attachment_file_id)
    try:
        await send_update_to_socket(message_document)
    except Exception as e:
...
async def sticker_message_from_user(message: types.Message) -> None:
    attachment_file_id = await upload_file_to_db_using_file_id(message.sticker.file_id)
    message_document = db.message.new_message(message, unread=True, attachment=attachment_file_id)
    try:
        await send_update_to_socket(message_document)
    except Exception as e:
...
async def document_message_from_user(message: types.Message) -> None:
    attachment_file_id = await upload_file_to_db_using_file_id(message.document.file_id)
    message_document = db.message.new_message(message, unread=True, attachment=attachment_file_id)
    try:
        await send_update_to_socket(message_document)
    except Exception as e:
